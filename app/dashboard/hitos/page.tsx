...
import { Star, Plus, Camera, Edit, Trash2, X, Save, Calendar, Clock, Search } from 'lucide-react'
...
export default function HitosPage() {
  ...
  const loadActivities = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const milestoneActivities = allActivities.filter((a: Activity) => a.type === 'milestone')
  ...
  useEffect(() => {
    if (selectedActivity && !isEditing) {
      ...
      setEditForm({
        title: selectedActivity.details?.title || '',
  ...
  const filteredActivities = activities.filter(act => {
    ...
    const title = act.details?.title?.toLowerCase() || ''
    return dateStr.includes(query) || notes.includes(query) || title.includes(query)
  })
            ...
            >
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-2xl font-bold text-gray-800">
                  {isEditing ? 'Editar Hito' : isDeleting ? 'Eliminar Hito' : 'Detalles del Hito'}
                </h3>
                <button
                  onClick={() => {
                    setSelectedActivity(null)
              ...
              {isDeleting ? (
                <div className="space-y-4">
                  <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                    <p className="text-red-800 font-semibold mb-2">¿Estás seguro de que quieres eliminar este hito?</p>
                    ...
                    <Button
                      onClick={async () => {
                        try {
                          ...
                          alert(error.message || 'Error al eliminar el hito')
                        }
                      }}
                    ...
                    <Button
                      onClick={async () => {
                        try {
                          ...
                          alert(error.message || 'Error al actualizar el hito')
                        }
                      }}
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <p className="text-lg font-semibold text-gray-800">
                        {selectedActivity.details?.title || 'Hito'}
                      </p>
                  ...
                  {isPremium ? (
                    ...
                    <div className="pt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                      <p className="text-sm text-yellow-800 mb-2">
                        <strong>Funcionalidad Premium:</strong> Actualiza a Premium para poder editar y eliminar hitos.
                      </p>
                      <Button
